...
import com.tn.shopping.config.KafkaSseConsumer;
import com.tn.shopping.config.KafkaSseProducer;
...
@RestController
@RequestMapping("/api/my-shopping-kafka")
public class MyShoppingKafkaResource {

    private final Logger log = LoggerFactory.getLogger(MyShoppingKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public MyShoppingKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
